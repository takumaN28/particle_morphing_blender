...
gltfLoader.load('./nObj.glb', (gltf) => {
    ...
    // モデルのロードが完了した後にパーティクルの属性を初期化
    initializeParticleAttributes(particles);

});



// 各パーティクルにランダムな位相を持たせる
function initializeParticleAttributes(particles) {
    const count = particles.geometry.attributes.position.count;
    const phases = new Float32Array(count * 3); // 各頂点ごとに位相を持たせる

    for (let i = 0; i < count * 3; i++) {
        phases[i] = Math.random() * 2 * Math.PI;
    }

    particles.geometry.setAttribute('phase', new THREE.BufferAttribute(phases, 3));
    console.log(particles.geometry.attributes.phase.array)
}
...
function updateParticles(particles, elapsedTime) {
    ...
    const amplitudeFactor = 0.006; // 偏りの強度係数（必要に応じて調整）
    ...
    for (let i = 0; i < positions.length; i += 3) {
        ...
        positions[i] += Math.cos(1.0 * elapsedTime + particles.geometry.attributes.phase.array[i]) * amplitude;
        positions[i + 1] += Math.sin(1.0 * elapsedTime + particles.geometry.attributes.phase.array[i]) * amplitude;
        positions[i + 2] += Math.sin(10.0 * elapsedTime + particles.geometry.attributes.phase.array[i]) * amplitude;
    }
